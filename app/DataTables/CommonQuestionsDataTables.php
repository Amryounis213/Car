...
class CommonQuestionsDataTables extends DataTable
{
    /**
    ...
    public function dataTable($query): EloquentDataTable
    {
        return datatables()
            ->eloquent($query)
            ->addIndexColumn()
            ->editColumn('title', function (CommonQuestion $model) {
                return $model->title;
            })
            ->editColumn('desc', function (CommonQuestion $model) {
                return $model->desc;
            })
            ->addColumn('status', function (CommonQuestion $model) {
                return view('admin.commonquestions.parts._status', compact('model'));
            })
            ->addColumn('action', function (CommonQuestion $model) {
                return view('admin.commonquestions.parts._action-menu', compact('model'));
            });
    }
    ...
    {
        return [
            Column::make('DT_RowIndex')->title(__('#'))->addClass('text-center')->orderable(false)->searchable(false),
            Column::make('title')->title(__('dashboard.title') )->addClass('text-center'),
            Column::make('desc')->title(__('dashboard.desc') )->addClass('text-center'),
